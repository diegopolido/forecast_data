...
RSpec.describe ForecastsController, type: :request do
  describe "GET /forecast" do
    ...
    context "when the ForecastService returns nil" do
      let(:zip_code) { "99999" }

      before do
        allow(ForecastService).to receive(:call).and_return(nil)
      end

      it "renders the form template with an error message" do
        get '/forecast', params: { zip_code: zip_code }
        expect(response).to have_http_status(:success)
        expect(response.body).to include("Unable to fetch forecast")
      end
    end
